...
public class SocketServer{
	...
	List<ThreadServer> clients = new ArrayList<ThreadServer>();
	...
	public void removeClient(ThreadServer client) {
		Iterator<ThreadServer> it = clients.iterator();
		while(it.hasNext()) {
			ThreadServer s = it.next();
			if(s == client) {
	...
	void cleanupClients() {
		...
		while(it.hasNext()) {
			...
			if(s.isClosed()) {
				...
				broadcast(new Payload(TypePayload.DISCONNECT, null));
				s.stopThread();
				it.remove();
			}
		}
	...
	void processMatch(String kvpClient, String fromClient, Entry<String, String[]> kvp) {
		...
		broadcast(new Payload(TypePayload.MESSAGE, message));
	}
