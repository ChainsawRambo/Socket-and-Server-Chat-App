...
public class ThreadServer extends Thread{
	...
	public ThreadServer(Socket myClient, String clientName, SocketServer server) throws IOException {
		...
		server.broadcast(new Payload(TypePayload.CONNECT, clientName), clientName);
		send(new Payload(TypePayload.MESSAGE, "Other players online: " + server.clients.size()));
		//send to my player my given name
		send(new Payload(TypePayload.UPDATE_NAME, clientName));
	}
	@Override
	public void run() {
		try{
	...
	void processPayload(Payload payload) {
		...
		switch(payload.payloadType) {
			case MESSAGE:
				Payload toClient = new Payload(TypePayload.MESSAGE,clientName + ": " + payload.message);
			...
			case DISCONNECT:
				...
				server.broadcast(new Payload(TypePayload.DISCONNECT,""));
				stopThread();
				break;
			case CHOICE:
